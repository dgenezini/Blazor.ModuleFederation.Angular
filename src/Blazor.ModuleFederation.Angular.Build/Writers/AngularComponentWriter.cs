...
";
        ...
        private const string PropertySetTemplate =
            @"        this._parameterValues['{0}'] = this.{0};";
        ...
        public static void Write(string outputDirectory, RazorComponentDescriptor componentDescriptor)
        {
            ...
            var propertiesSetList = GetPropertiesSetList(componentDescriptor.Parameters, jsTypeNames);

            var componentContents = string.Format(AngularComponentTemplate, 
                angularComponentName, componentDescriptor.Name, propertyList, propertiesSetList);

            var angularComponentFolder = $"{outputDirectory}/src/app/{angularComponentName}";
        ...
        private static string GetPropertiesSetList(
            IReadOnlyList<BoundAttributeDescriptor> parameters, string[] jsTypeNames)
        {
            Debug.Assert(parameters?.Count == jsTypeNames?.Length);
            ...
            for (var i = 0; i < parameters?.Count; i++)
            {
                ...
                if (typeName != "eventcallback")
                {
                    var line = string.Format(PropertySetTemplate, camelCaseName);
